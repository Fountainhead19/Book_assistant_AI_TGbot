...
def get_stream_cutoff_values(update: Update, content: str) -> int:
    ...
    return 4096 if len(content) > 1000 else 1000 if len(content) > 200 \
        else 200 if len(content) > 50 else 50 if len(content) > 15 \
        else 15 if len(content) > 1 else 7 if len(content) == 1 else 50
...
async def is_allowed(config, update: Update, context: CallbackContext, is_inline=False) -> bool:
    ...
    """
    channel_id = '@libraiservice'
    user_id1 = update.message.from_user.id
    с = await context.bot.get_chat_member(chat_id=channel_id, user_id=user_id1)
    if с["status"] != 'left':
        return True
    else:
         return False

async def is_allowed_prem(config, update: Update, context: CallbackContext, is_inline=False) -> bool:
    """
    Checks if the user is allowed to use the bot.
    """
    ...
    # Создание маски для выбора строк, удовлетворяющих условиям: пользователь соответствует и прошло больше 30 дней
    mask = (df['User'] == user_name) & ((current_time - df['Date Added']) > pd.Timedelta(days=30))
    mask1 = (df['User'] == user_name) & ((current_time - df['Date Added']) <= pd.Timedelta(days=30))
    if mask.any():
        # Удаляем строки, удовлетворяющие условиям
        df = df[~mask]
