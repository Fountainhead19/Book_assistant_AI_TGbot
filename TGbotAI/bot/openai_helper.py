...
GPT_4_128K_MODELS = ("gpt-4-1106-preview","gpt-4-turbo","gpt-4-turbo-2024-04-09","gpt-4o-2024-05-13")
...
def are_functions_available(model: str) -> bool:
    ...
    if model in ("gpt-3.5-turbo", "gpt-3.5-turbo-1106", "gpt-4", "gpt-4-32k","gpt-4-1106-preview","gpt-4-turbo-preview"):
...
class OpenAIHelper:
    ...
    async def get_chat_response_stream(self, chat_id: int, query: str, model_conf: int):
        """
        ...
        """
        plugins_used = ()
        response = await self.__common_get_chat_response(chat_id, query, model_conf = model_conf, stream=True)
        if self.config['enable_functions'] and not self.conversations_vision[chat_id]:
    ...
    async def __common_get_chat_response(self, chat_id: int, query: str, model_conf: int, stream=False):
        """
        ...
        try:
            ...
            if  model_conf == 1:
                self.config['model'] = os.environ.get('OPENAI_MODEL', 'gpt-3.5-turbo')
                logging.warning(f'3.5')
            else : 
                self.config['model'] = os.environ.get('OPENAI_MODEL', 'gpt-4-turbo-2024-04-09')
                logging.warning(f'4')
            common_args = {
